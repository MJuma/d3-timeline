...
var import_d3_axis = require("d3-axis");
var import_d3_array = require("d3-array");
var import_d3_brush = require("d3-brush");
...
var colors = (0, import_d3_scale.scaleOrdinal)(import_d3_scale_chromatic.schemeAccent);
...
function timeline() {
  ...
  const timeline2 = {
    ...
    render: function(domElement) {
      ...
      if (options.showTooltips && (0, import_d3_selection.select)("#timeline-tooltip").empty()) {
        createTooltipHost();
      }
      timelineXScale = createXScale(options);
      brushXScale = createXScale(options);
...
function createTooltipHost() {
  (0, import_d3_selection.select)("body").append("div").attr("id", "timeline-tooltip").style("position", "absolute").style("color", "black").style("background-color", "#FFFFEE").style("border", "1px solid").style("padding", "4px").style("visibility", "hidden");
}
function createChartContainer(domElement, options) {
  const outerContainer = (0, import_d3_selection.select)(domElement).append("div").attr("id", "timeline-outer-container");
...
function createXScale(options) {
  const domainExtent = (0, import_d3_array.extent)([...options.data.map((d) => new Date(d.start)), ...options.data.map((d) => new Date(d.end ?? ""))]);
  let rangeEnd = options.width - 16;
  const rangeEndMin = rangeEnd / 2;
  const scale = (0, import_d3_scale.scaleTime)().domain(domainExtent).range([16, rangeEnd]);
  for (let i = 0; i < 2; ++i) {
    let textEnd = 0;
...
function updateChart(chart, timelineXScale, brushXScale, options) {
  const groupMap = (0, import_d3_array.group)(options.data, (d) => d.group);
  ...
  chart.select("#timeline-groups").selectAll("div").data(groupMap).join(
    function enter(enter) {
      ...
      enter.each(function(g) {
        (0, import_d3_selection.select)(`#perfbox-${normalizeGroupName(g[0])}`).remove();
      ...
      svg.append("g").attr("class", "group-label").append("text").style("font", "11px sans-serif").attr("transform", "translate(0, 50) rotate(270)").attr("x", groupHeight / 2 * -1).attr("y", 12).text((g) => g[0]).each(function() {
        const self = (0, import_d3_selection.select)(this);
...
function updatePerfBox(chart, groupMap, options) {
  const perfTimelineHeight = getPerfChartHeight(options);
  const perfTimelineData = new import_d3_array2.InternMap();
    ...
    chart.select(svgId).selectAll("g").data(perfTimelineData.get(group2) ?? []).join(
      function enter(enter) {
        return enter.append("g").each(function() {
          (0, import_d3_selection.select)(this).append("rect");
        });
      },
      function update(update) {
    ...
    chart.select(groupId).selectAll("g.datapoint").data(groupData, (d) => `${d.start.toString()}-${d.id}`).join(
      function enter(enter) {
        return enter.append("g").attr("class", (d) => d.end || d.start === d.end ? "datapoint interval" : "datapoint instant").each(function(d) {
          const node = (0, import_d3_selection.select)(this);
...
function updateTooltips(svgGroups, groupMap, options) {
  ...
  if (options.showTooltips) {
    const tooltip = (0, import_d3_selection.select)("#timeline-tooltip");
...
function updateAxis(chart, timelineXScale, options) {
  if (options.xAxis) {
    ...
    if (options.data.length > 0) {
      const xAxis = (0, import_d3_axis.axisBottom)(timelineXScale).ticks(10);
...
function updateBrush(chart, brushXScale, timelineXScale, groupMap, options) {
  if (options.xBrush) {
    ...
    const brush = (0, import_d3_brush.brushX)().extent([[16, 0], [brushXScale.range()[1], brushHeight]]).on("brush", handleBrush).on("end", handleBrush);
    ...
    if (options.xAxis) {
      ...
      const xAxis = (0, import_d3_axis.axisBottom)(brushXScale).ticks(10);
